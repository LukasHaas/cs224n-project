...
from torch.nn import Module, ModuleList, MultiheadAttention, Linear, Dropout, LayerNorm
from typing import Optional

class CustomTransformerEncoder(Module):
    r"""A custom transformer encoder supporting attention outputs.
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        for mod in self.layers:
            ...
            if all_attention_weights is None:
                all_attention_weights = clone(attention_weights)
            else:
...
class CustomTransformerEncoderLayer(Module):
    r"""A custom transformer encoder layer which returns attention scores.
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        x = src
        x_new, attention_weights = self._sa_block(self.norm1(x), src_mask, src_key_padding_mask)
        if self.norm_first:
            x = x + x_new
